...
//main
var data=Array();//real data
var showing=Array();//index number of data that showing
var not_showing=Array();//index number of data that not_showing

var showing_backup=Array();//will be a list contain all index number(defect show all)
...
$(document).ready ( function(){
	...
	showing_backup=[...showing];//make copy(not shadow)
	...
	$('div.history').on('click', 'div.contain', function(event) {
		produce_recovery(parseInt(event.target.id));
	});
	
	$('div.history').on('click', 'button.cancel', function(event) {
		search_history.splice(parseInt(event.target.id),1);
		if((search_history.length)===0){//delete the last element(all element deleted) return to original state
			showing=[...showing_backup];//make copy(not shadow)
			not_showing=Array();//clear
			time_searching=0;//clear
			//search_history=Array();already clear
			$(".history").html("You haven't search any word yet");
		}
		produce_recovery((search_history.length-1));//number of item to index number:-1,
	});
});
...
function produce_recovery(index){
	...
	for(var i = 0; i <= index; i++)//rerun all instructions
	{
...
function display_history(key, instruction){
	if(time_searching==0)
	{
		$(".history").html("<div class='contain' id='0'>"+instruction+key+"</div><button class='cancel' id='0'>X</button><br />");
		time_searching+=1;
	}
	else
	{
		$(".history").html($(".history").html()+"<div class='contain' id='"+time_searching+"'>"+instruction+key+"</div><button class='cancel' id='"+time_searching+"'>X</button><br />");
		time_searching+=1;
	}
}
